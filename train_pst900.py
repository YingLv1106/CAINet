...
from toolbox.losses import CrossEntropyLoss2d, CrossEntropyLoss2dLabelSmooth, ProbOhemCrossEntropy2d, FocalLoss2d, \
    LovaszSoftmax, LDAMLoss, CCLoss1, KLLoss, ResizeCrossEntropyLoss, OhemCELoss, MAE, MSE
...
def run(args):
    ...
    # criterion_att3 = KLLoss()
    # criterion_att4 = KLLoss()
    # criterion_att3 = MAE()
    # criterion_att4 = MAE()
    criterion_att3 = MSE()
    criterion_att4 = MSE()
    ...
    for ep in range(cfg['epochs']):
        ...
        for i, sample in enumerate(train_loader):
            ...
            logger.info(
                f'Iter | [{ep + 1:3d}/{cfg["epochs"]}] lr={optimizer.param_groups[0]["lr"]:.12f} train/val loss={train_loss_meter.avg:.5f}/{val_loss_meter.avg:.5f}, '
            ...
            def save_ckpt(logdir, ind, model):
                state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
                torch.save(state, os.path.join(logdir, ind + 'model.pth'))
